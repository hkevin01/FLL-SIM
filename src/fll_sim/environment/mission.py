...
class FLLMissionFactory:
    """
    Factory for creating standard FLL missions based on real
    ...
    def create_submerged_2024_missions() -> List[Mission]:
        ...
        coral_nursery = Mission(
            ...
            name="Coral Nursery",
            description=(
                "Transport coral samples to the nursery area for cultivation"
            ),
            ...
            hint=MissionHint(
                strategy=(
                    "Use precise movements to carefully transport coral "
                    "without dropping it"
                ),
                tips=[
        ...
        shark_mission = Mission(
            ...
            description=(
                "Guide the shark to safe waters while avoiding obstacles"
            ),
            ...
            hint=MissionHint(
                strategy=(
                    "Use ultrasonic sensor to navigate around obstacles "
                    "while guiding shark"
                ),
                tips=[
        ...
        kraken_mission = Mission(
            ...
            description=(
                "Navigate through the kraken's tentacles to reach the treasure"
            ),
            ...
            hint=MissionHint(
                strategy=(
                    "Use precise navigation and color detection to reach "
                    "exact position"
                ),
                tips=[
...
"""
        ...
        speed_mission = Mission(
            ...
            description=(
                "Navigate underwater course at high speed while maintaining "
                "control"
            ),
            ...
            hint=MissionHint(
                strategy=(
                    "Maintain high speed while navigating precisely to "
                    "target area"
                ),
                tips=[
    ...
    def create_line_following_mission(
        ...
        return Mission(
            ...
            description=(
                f"Follow the {line_color} line for {track_length}mm with "
                f"max deviation {max_deviation}mm"
            ),
...
class MissionManager:
    ...
    def start_mission(self, mission_id: str) -> bool:
        ...
        if (self.active_mission and
                self.active_mission.status == MissionStatus.IN_PROGRESS):
            logger.warning(
                f"Stopping current mission: {self.active_mission.name}"
            )
    ...
    def update_active_mission(
        ...
        if (self.active_mission and
            ...
            if self.active_mission.status == MissionStatus.COMPLETED:
                ...
                logger.info(
                    f"Mission completed! Total score: {self.total_score}"
                )
    ...
    def get_session_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'completion_rate': (
                len(self.completed_missions) / len(self.missions)
